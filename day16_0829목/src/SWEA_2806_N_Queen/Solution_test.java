...
public class Solution_test {	
	...
	static void putQ(int r) {
		...
		for(int i = 0; i < N; i++) {
			flag = false;
			if(!visitedR[i] || !visitedC[i]) break;
			flag = true;
		}
...
out:	for(int c = 0; c < N; c++) {
			// 가지 치기
			// 1. 가로 세로 체크
			if(visitedC[c] != false || visitedR[r] != false) continue; // 이미 방문한 r이거나 c면 
			
			// 2. 대각선 체크
			for(int d = 1; d < N; d++ ) {
				// 왼쪽 위 -> r, c 둘 다 -
				if(r - d >= 0 && c - d >= 0 // 범위 내에서 대각선 탐색을 하는데, 
				&& visitedR[r - d] != false // 만약 이미 퀸이 있다면 불가능 
				&& visitedC[c - d] != false) // 퀸이 없으면 & 연산이 0으로 나옴
					continue out;
				// 오른쪽 위 -> r -, c +
				if(r - d >= 0 && c + d < N  
				&& visitedR [r - d] != false 
